...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = isAvatar
      ? `photorealistic casual selfie photo of ${rawDesc || "a beautiful young woman"}, natural smartphone photo taken with a front camera, head and shoulders perfectly centered filling the frame, camera directly facing her, realistic human skin with visible pores, natural texture, tiny imperfections and subtle asymmetry, detailed iris with natural highlights, individual eyelashes, softly shaped natural brows, light natural makeup, slight natural shine on the skin, soft natural window light, gentle catchlights in the eyes, shallow depth of field, softly blurred neutral background, tack sharp focus on the eyes, high resolution face detail, square profile picture crop, no full body, no chest, no cleavage, candid real photography, must look like a real photo of a real person, NOT a CGI render, no plastic skin, no wax skin, no airbrushed porcelain face, no doll-like mannequin face, no anime, no illustration`
