...
const DATA = {
  ...
  vera: { seed: 3001, face: "beautiful ethereal pale white woman in her 20s, angular symmetrical face, high sculpted cheekbones, intense dark piercing eyes, full seductive lips, straight elegant nose, mysterious femme fatale expression, slim waist, very wide hips, very large natural breasts, porcelain flawless skin, smoky dark eye makeup, neutral lips",
    hair: { moreno: "long straight dark brown hair with subtle highlights", rubio: "long straight platinum blonde icy hair", pelirrojo: "long straight dark red burgundy hair", rosa: "long straight pastel pink hair" },
    outfit: { elegante: "wearing sophisticated black lace lingerie with garter belt, dark mysterious allure", casual: "wearing a black leather jacket over a lace top and ripped skinny jeans, boots", fiesta: "wearing a sultry dark red velvet dress with deep neckline and slit", "bikini-suave": "wearing a black string bikini, pale skin contrasting dark fabric" },
    background: { "neon-room": "standing in a dark moody room with pink neon accent lighting, shadows, mysterious atmosphere", "beach-night": "standing on a moonlit beach, dark waves, misty night atmosphere, dramatic clouds", studio: "standing in a high-end fashion studio with dramatic directional lighting and black backdrop", "car-night": "standing next to a sleek black sports car in a dark alley, neon reflections, film noir mood" } },
  alma: { seed: 4001, face: "beautiful thick voluptuous latina woman in her 20s, heart-shaped symmetrical face, big warm dark brown eyes, full pouty lips, cute upturned nose, warm inviting smile, extreme hourglass figure, very slim waist, extremely wide hips, XXL massive natural breasts, tan glow skin, glossy pink lip gloss, warm dewy makeup",
  ...
  kira: { seed: 5001, face: "beautiful edgy white woman in her 20s, sharp angular symmetrical face, piercing ice blue eyes, full rebellious lips, straight nose, cyberpunk aesthetic vibe, fit athletic body, slim toned waist, wide hips, large natural breasts, pale skin, dark smokey eye makeup, alternative style",
  ...
  maya: { seed: 6001, face: "beautiful glamorous white woman in her 20s, perfectly symmetrical doll-like face, sparkling hazel eyes, full pouty lips with gloss, perfect straight nose, influencer aesthetic, fit hourglass figure, slim waist, wide hips, very large natural breasts, sun-kissed bronzed skin, full glam makeup, laminated brows",
    hair: { moreno: "long straight dark brown hair with balayage blonde highlights", rubio: "long straight platinum blonde silky hair with subtle highlights", pelirrojo: "long straight red hair with balayage highlights", rosa: "long straight neon pink hair with baby lights" },
    outfit: { elegante: "wearing luxurious black lace lingerie set with garter belt, glamorous pose", casual: "wearing a tight white crop top with matching high-waist yoga leggings, fit vibe", fiesta: "wearing a glittering gold bodycon mini dress with plunging V-neckline", "bikini-suave": "wearing a black string bikini set, influencer beach aesthetic" },
    background: { "neon-room": "standing in a pink neon-lit bedroom with luxury decor, mirror wall, glamorous vibe", "beach-night": "standing on a luxury tropical beach at night, moonlit ocean, VIP vibe", studio: "standing in a professional photo studio with ring light and white backdrop, influencer shoot", "car-night": "standing next to a white luxury sports car at night, city lights, glamorous street setting" } },
  sasha: { seed: 7001, face: "beautiful curvaceous black woman in her 20s, perfectly round symmetrical face, big captivating dark brown eyes, full luscious lips, cute button nose, confident powerful presence, extreme hourglass figure, very slim waist, extremely wide hips, XXL massive natural breasts, smooth dark chocolate skin with natural sheen, glossy nude lip gloss, subtle glowing highlighter",
  ...
  yuki: { seed: 8001, face: "beautiful exotic asian woman in her 20s, oval delicate symmetrical face, big almond-shaped dark eyes with natural winged eyeliner, full soft pink lips, small elegant nose, cute yet seductive expression, hourglass figure, slim waist, wide hips, large natural breasts, smooth fair porcelain skin with dewy finish, subtle glossy pink makeup, blushed cheeks",
    hair: { moreno: "long straight jet black silky hair with subtle blue shine", rubio: "long straight platinum blonde silky hair", pelirrojo: "long straight dark cherry red silky hair", rosa: "long straight pastel pink silky hair" },
    outfit: { elegante: "wearing delicate black lace lingerie set with garter belt, innocent yet seductive", casual: "wearing a very tight white cropped baby tee and low-rise denim skirt, cute and sexy", fiesta: "wearing a stunning black satin mini dress with side cutouts and high slit", "bikini-suave": "wearing a minimal black string bikini set, porcelain skin glowing" },
    background: { "neon-room": "standing in a pink neon-lit room with paper lanterns, cute kawaii aesthetic", "beach-night": "standing on a moonlit tropical beach, gentle waves, romantic atmosphere", studio: "standing in a clean professional studio with soft pink lighting and white backdrop", "car-night": "standing next to a sleek black sports car at night, Tokyo-style neon cityscape" } },
};

async function genOne(girlId, hk, ok, bk) {
  const g = DATA[girlId];
  const filePath = path.join(ROOT, girlId, `${hk}_${ok}_${bk}.jpg`);
  if (fs.existsSync(filePath)) return { ok: false, file: `${girlId}/${hk}_${ok}_${bk}.jpg`, skipped: true };
  const prompt = `${g.face}, ${g.hair[hk]}, ${g.outfit[ok]}, ${g.background[bk]}, ${SUFFIX}`;
  const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=864&height=1024&seed=${g.seed}&nologo=true&model=flux`;
  try {
    ...
    return { ok: true, file: `${girlId}/${hk}_${ok}_${bk}.jpg`, size: buf.length };
  } catch (e) {
    return { ok: false, file: `${girlId}/${hk}_${ok}_${bk}.jpg`, err: e.message.slice(0, 60) };
  }
}
...
async function main() {
  // Build task list
  const tasks = [];
  for (const girlId of Object.keys(DATA)) {
  ...
  for (let i = 0; i < tasks.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(batch.map(t => genOne(t.girlId, t.hk, t.ok, t.bk)));
    done += results.length;
    for (const r of results) {
      if (!r) continue;
      if (r.skipped) continue;
      if (r.ok) { generated++; process.stdout.write(`✓ ${r.file} (${(r.size/1024).toFixed(0)} KB)\n`); }
      else if (r.err && r.err !== "HTTP 500") { failed++; process.stdout.write(`✗ ${r.file}: ${r.err}\n`); }
    }
    if (done % 30 === 0) {
      const elapsed = ((Date.now() - start) / 1000).toFixed(0);
      process.stdout.write(`  ${done}/${total} checked, ${generated} new, ${failed} failed (${elapsed}s)\n`);
    }
  ...
  console.log(`\nDone! ${generated} generated, ${failed} failed, in ${elapsed} min`);

  // Summary
  for (const girlId of Object.keys(DATA)) {
    const cnt = fs.readdirSync(path.join(ROOT, girlId)).filter(f => f.endsWith('.jpg')).length;
