...
export default function HistoryPage() {
  ...
  function openGirl(row: GirlRow) {
    setSelected(row);
    const hist = getConversationHistory(row.girlId);
    setTranscript(hist.length > 0 ? hist : []);
  }
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1.5 text-sm text-muted/70">
              {selected ? `Conversación con ${selected.name}` : "Toca una chica para ver solo su historial"}
            </p>
          </div>
          {!selected && rows.some((r) => r.lastTs > 0) && (
            <button
