...
export default function CustomGirlCard({ data, onDelete }: { data: CustomGirlData; onDelete?: () => void }) {
  ...
  return (
    <div className="group character-card overflow-hidden relative">
      <Link href={`/chat/luna?custom=${data.id}`} onClick={handleClick} className="block">
      ...
      <button
        onClick={(e) => { e.stopPropagation(); deleteCustomGirl(data.id); onDelete?.(); }}
        className="absolute right-2 top-2 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-red-600 border-2 border-red-400 text-white shadow-lg hover:bg-red-500 transition-all active:scale-90"
        title="Eliminar"
      >
      ...
      <div className="flex items-center gap-1.5 p-3 sm:p-4">
        <Link href={`/chat/luna?custom=${data.id}`} onClick={handleClick} className="btn-primary flex h-9 flex-1 items-center justify-center text-[0.6rem] font-bold sm:text-xs">
          Chatear
        </Link>
      </div>
    </div>
