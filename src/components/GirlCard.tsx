  ...
  return (
    <div className="group character-card overflow-hidden">
      <Link href={`${basePath}/customize/${girl.id}`} className="block">
        <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#12121a]">
          ...
          <div className="absolute left-3 right-3 top-3 flex items-start justify-between">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); e.preventDefault(); window.location.href = `${basePath}/customize/${girl.id}`; }}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5">
            <h3 className="font-black leading-none tracking-tighter text-white sm:leading-[28px]"
              style={{ fontSize: "clamp(18px, 5vw, 25px)", textShadow: "0 2px 12px rgba(0,0,0,0.7)", letterSpacing: "-0.02em" }}
            >
              {girl.name} <span className="font-bold text-white/90">{girl.age}</span>
            </h3>
            <p className="mt-1.5 leading-snug text-white/85 line-clamp-2 sm:leading-snug"
              style={{ fontSize: "clamp(12px, 3vw, 15px)", textShadow: "0 1px 8px rgba(0,0,0,0.6)" }}
            >
              {girl.story}
            </p>
          </div>
      ...
      <div className="flex items-center gap-1.5 p-3 sm:p-4">
        ...
        <Link href={`${basePath}/call/${girl.id}`} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-green-500/30 bg-green-500/20 text-green-400 transition hover:bg-green-500/30 sm:w-10" title="Llamada">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
        </Link>
        <Link href={`${basePath}/call/${girl.id}`} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-pink-500/30 bg-pink-500/20 text-pink-400 transition hover:bg-pink-500/30 sm:w-10" title="Videollamada">
