  ...
  return (
    <div className="girl-card group">
      <div className={`relative aspect-[4/3] w-full bg-gradient-to-b ${gradient} overflow-hidden`}>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#08030f] via-[#08030f]/40 to-transparent opacity-90" />
      ...
      <div className="px-5 pb-5 pt-4">
        <div className="mb-2 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold tracking-tight">{girl.name}</h3>
            <p className="text-xs text-muted/70">{girl.style}</p>
          </div>
        </div>
        ...
        <p className="mb-4 text-xs text-muted/70 leading-relaxed">{girl.personalityLabel}</p>
        <div className="flex flex-col gap-2">
          <Link href={`/chat/${girl.id}`}>
            <button className="w-full rounded-xl gradient-btn py-2.5 text-sm font-semibold transition-all duration-200 hover:shadow-lg hover:shadow-pink-500/25 active:scale-[0.98] flex items-center justify-center gap-2">
          ...
          <div className="flex gap-2">
            <Link href={`/call/${girl.id}`} className="flex-1">
              <button className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-white/5 py-2.5 text-xs font-semibold transition-all duration-200 hover:bg-white/10 hover:border-white/20 active:scale-[0.98]">
