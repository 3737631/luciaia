  ...
  if (callState === "ringing" || callState === "connecting") {
    ...
    return (
      <>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, overflow: "hidden", background: "#08050a", overscrollBehavior: "none", touchAction: "manipulation", fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display','Helvetica Neue',Arial,sans-serif", WebkitUserSelect: "none", userSelect: "none", display: "grid", gridTemplateRows: "auto 1fr auto", minHeight: "100dvh", opacity: 1, transition: "opacity 0.2s ease" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 1, transform: "translateY(-14px)", minHeight: 0, padding: "0 24px" }}>
            <div style={{ position: "relative", width: "min(180px, calc(100vw - 200px), 184px)", height: "min(180px, calc(100vw - 200px), 184px)", flexShrink: 0 }}>
              {callState === "ringing" && <div style={{ position: "absolute", inset: -7, borderRadius: "50%", border: "1px solid rgba(180,50,100,0.18)", transform: "scale(1)", opacity: 0.3, pointerEvents: "none", transition: "transform 0.3s ease, opacity 0.3s ease" }} />}
            ...
            <div style={{ marginTop: 13, fontSize: 17, fontWeight: 400, color: "rgba(255,255,255,0.68)", textAlign: "center", lineHeight: 1.25, height: 22, transition: "opacity 0.18s ease" }}>{callState === "ringing" ? "Llamando" : "Conectando"}</div>
