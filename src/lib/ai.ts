// Puter.js se carga via script tag en layout.tsx
// window.puter está disponible globalmente
...
export function isPuterSignedIn(): boolean {
  try {
    return (window as any).puter?.auth?.isSignedIn() === true;
  } catch {
    return false;
  }
}

export function signInToPuter(): void {
  try {
...
export async function getAiResponse(
  ...
  if (!puter) throw new Error("Puter.js no está cargado");
  if (!puter.auth?.isSignedIn()) throw new Error("auth_required");
